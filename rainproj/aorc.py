...
class AORC:
    ...
    def download_aorc():
        ...
        aorc = xr.open_mfdataset(urls,engine='zarr',drop_variables=['DLWRF_surface','DSWRF_surface','PRES_surface','SPFH_2maboveground','TMP_2maboveground','UGRD_10maboveground','VGRD_10maboveground'])['APCP_surface']
        return aorc

def _prep_download(output_dir=f'{WORK}/hindcast/aorc'):
    dateformat = "%Y-%m-%d %H:%M:%S"
    print(f'{datetime.now().strftime(dateformat)}: Preparing')
    result = AORC.download_aorc().sortby('latitude').rename({'latitude':'lat','longitude':'lon'}).pipe(RainReader2.slice_rain,negative_lon=True).chunk(time=TimeResampler("MS"))#.pipe(RainReader2.resample_daily)
    ...
    lats = np.arange(25.0,49.5+.25,.25)
    lons = np.arange(235.0,293.0+.25,.25)
    ...
    month_year_ds_groups_dict = {f'{output_dir}/{pd.Timestamp(timestamp).strftime('%Y-%m')}_aorc.parquet':month_year_ds for timestamp,month_year_ds in month_year_ds_groups}
    month_year_ds_groups_dict = {filepath:ds for filepath,ds in month_year_ds_groups_dict.items() if not Path(filepath).is_file()}
...
def run_download():
    ...
    task_generator = joblib.Parallel(n_jobs=min(128,os.cpu_count()),timeout=99999,return_as='generator_unordered',verbose=100)(joblib.delayed(process_month)(filename, month_year_ds) for filename, month_year_ds in month_year_ds_groups.items())
...
def run_download5(workers=os.cpu_count(),nbatchs=3,output_dir=f'{WORK}/hindcast/aorc'):
    ...
    month_year_ds_batchs = [month_year_ds_groups[batch_num*batch_size:(batch_num+1)*batch_size] for batch_num in range(0,nbatchs)]
